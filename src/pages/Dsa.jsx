...
const sidebarLinks = [
  ...
  },
  {
    ...
    path: "/dsa/String",
    icon: "🎨",
    description: "String data structure",
...
const DSA = () => {
  ...
  const currentPageInfo = useMemo(() => {
    let currentPath = location.pathname + "/" + "array";
    ...
    // console.log(currentPath , currentLink , currentIndex ,pageName)
    ...
    return {
      ...
      progress:
        currentIndex >= 0
          ? ((currentIndex + 1) / sidebarLinks.length) * 100
          : 0,
    };
  }, [location.pathname]);
    ...
    >
      ...
      {isActive && (
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
          <div
            className={`w-2 h-2 bg-gradient-to-r ${link.color} rounded-full animate-pulse`}
          ></div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row gap-6 mt-[20%] lg:mt-[6%] relative">
      {/* Mobile Menu Button */}
      <button
        className="lg:hidden fixed top-[] text-5xl right-6 z-50 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-3 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
        ...
        <div className="relative">
          ...
          <nav className="flex flex-col gap-2 mb-6">
            {sidebarLinks.map((link, index) => {
              const isActive =
                location.pathname === link.path || // exact match
                location.pathname.startsWith(`${link.path}/`) || // nested route
                (location.pathname === "/dsa" && link.path === "/dsa/array"); // special case
            ...
            <div className="mt-4 p-3 bg-gradient-to-r from-purple-800/20 to-pink-800/20 rounded-lg border border-purple-700/30">
              ...
              <div className="flex items-center gap-2">
                <span className="text-lg">
                  {currentPageInfo.currentLink.icon}
                </span>
          ...
          <div className="mb-8">
            ...
            {currentPageInfo.currentLink && (
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h1 className="text-4xl lg:text-2xl font-bold text-white">
                    {currentPageInfo.currentLink.name}
                  </h1>
                  <p className="text-gray-400 lg:text-sm text-2xl">
                    {currentPageInfo.currentLink.description}
                  </p>
                </div>
              </div>
            )}
